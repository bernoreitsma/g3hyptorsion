...
Jacobian of C, returns the image of P+Q.}
  ...
  if assigned C`KummerBQF then
    ...
    fs := Coefficients(f);  
    fs cat:= [0 : i in [#fs+1..9]];
    BB := K3biquforms(fs);	// this command costs 4 seconds...
    C`KummerBQF := BB;
  end if;
...
/* The design of Multiple() is based on [5], section 4, but for genus 3. 
A good way of understanding what it does is to run through the binary
expansion of n from right to left (taking index i, let's say the length of this string is k), 
add 2^i * Pz to Px if  we encounter a '1', and add to the difference between 
Px and 2^k otherwise.
*/
